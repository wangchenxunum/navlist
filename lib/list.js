...
async function create({parent, title, link, flink, glink, style, openWith, inSub, hide, order, explain}, group) {
	...
	let {insertedIds} = await this.db.list.insert({parent, group, title, link, flink, glink, style, openWith, inSub, hide, order, explain});
	if (!insertedIds) {
...
async function set(id, {parent, title, link, flink, glink, style, openWith, inSub, hide, order, explain}, group) {
	...
	let {result} = await this.db.list.update({_id: id, group}, {$set});
...
async function remove(id, group) {
	...
	let {result} = await this.db.list.deleteMany({_id: id, group});
...
async function get(id, group) {
	...
	if (Array.isArray(id)) {
		...
		const info = await this.db.list.find(condition).toArray();
		info.forEach(info=>{
			info.id = info._id;
			delete info._id;
		});
	...
	condition._id = id;
	const info = await this.db.list.findOne(condition);
	if (!info) {
		return null;
	}
	info.id = info._id;
...
async function list({parent, group, title, link, flink, glink, openWith, inSub, hide, skip = 0, limit = 30,} = {}) {
	...
	const cursor = this.db.list.find(condition).skip(skip).limit(limit);
 ...
 * @param  {Boolean}				options.transform	是否将glink转换，默认值为true
 * @param  {Boolean}				options.filter		是否过滤无效导航，默认值为false
 * @param  {Boolean}				options.sort		是否根据order排序，默认为true
 * @return {NavInfo[]}									导航列表
 */
async function subList(id, { hasParent, hide, transform = true, filter = false, sort = true, } = {}) {
	if (!(id === null || (id = getId(id)))) {
		throw new OperationError("Id无效");
	}
	let parent;
	if (id && hasParent !== false) {
		parent = await this.db.list.findOne({_id: id});
		if (!parent) {
			return [];
		}
	}
	...
	const list = await this.db.list.find(condition).toArray();
	...
	if (transform) {
		...
		const group = await this.group.get();
		...
		list.forEach(info => {
			...
			for (let i = 0; i < l; i++) {
				if (info.glink === group[i].group) {
					info.gtitle = group[i].title;
					return ;
				}
			}
		});
